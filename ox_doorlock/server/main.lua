...
local function isAuthorised(playerId, door, lockpick)
    ...
    if player then
        ...
        -- Convert `door.groups` to the array format if necessary
        if type(door.groups) == "table" and not door.groups[1] then
            local groupArray = {}
            for job, grade in pairs(door.groups) do
                table.insert(groupArray, { name = job, grade = grade })
            end
            door.groups = groupArray
        end
        ...
        if authorised and door.passcode then
            authorised = door.passcode == lib.callback.await('ox_doorlock:inputPassCode', playerId)
        end
    end
